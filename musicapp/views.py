...
def index(request):
    ...
    if not request.user.is_anonymous:
        ...
        if last_played_list:
            ...
            first_time = True
            last_played_song = Song.objects.get(id=30)
...
def play_next_song(request):
    # Get the list of all song IDs
    all_song_ids = Song.objects.values_list('id', flat=True)
    random_song_id = random.choice(all_song_ids)
    songs = Song.objects.filter(id=random_song_id).first()
    # Add data to recent database
    if list(Recent.objects.filter(song=songs,user=request.user).values()):
        data = Recent.objects.filter(song=songs,user=request.user)
        data.delete()
